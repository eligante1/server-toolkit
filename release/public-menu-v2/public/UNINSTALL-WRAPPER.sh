...
readonly script="$bootstrap_dir/uninstall-server-toolkit-1.0.0.sh"
...
printf '%s  %s\n' 'fa791fdfb6038f352d2aec5894e1e2bbb1917eff7a676c350d5f180f7be336b3' "$script" | /usr/bin/sha256sum -c -
...
if [[ ! -x /usr/bin/pgrep ]]; then
  ...
  /usr/bin/pgrep -f -- "$bootstrap_dir|/usr/local/bin/server-toolkit|/usr/local/bin/server-toolkit-audit-v1|/usr/local/lib/server-toolkit|/usr/local/lib/\.server-toolkit-txn-7e7c1cdda1ef|/usr/local/bin/\.server-toolkit-txn-7e7c1cdda1ef" >/dev/null 2>&1
  pgrep_rc=$?
  case "$pgrep_rc" in
...
if (( rc == 0 )); then
  for path in \
    ...
    /usr/local/lib/.server-toolkit-txn-7e7c1cdda1ef \
    ...
    /usr/local/bin/.server-toolkit-txn-7e7c1cdda1ef-server-toolkit-audit-v1.sha256; do
